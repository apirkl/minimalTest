...
class GithubController(
    ...
    @Get("/proxy")
    fun proxiedClient(request: HttpRequest<String>): Publisher<MutableHttpResponse<*>>? {
        return proxyHttpClient.proxy(request.mutate()
            .uri(UriBuilder.of(URI("https://docs.github.com/rest/overview/resources-in-the-rest-api#rate-limiting")).build())
            .header(USER_AGENT, "Micronaut HTTP Client")
        )
    }

    @Get("/slowProxy")
    fun slowProxiedClient(): Publisher<MutableHttpResponse<*>>? {
